...
n_per_group <- 20                    # Sample size per group (treatment and control)
...
# only print proportion of significant outliers 
write.table(as.data.frame(Master_summary %>%
                            ...
                            tidyr::pivot_wider(names_from = subgroup,values_from=prop_3sigma)),row.names=F,sep="\t",quote = F,
            file=paste0(current_wd,"/HTE_small.txt"))
...
current_wd <- getwd()
plot_path <- file.path(current_wd,"/heterogeneity_plots_small.pdf")
...
grid.text("Impact of HTE on LnCVR in small Samples (n=20/arm): A Simulation Study",
...
grid.text("Impact of HTE on %LnCVR (p<=0.05) in small Samples: A Simulation Study",
          y = unit(1, "npc") - unit(0.00, "inches"),
          gp = gpar(fontsize = 12, fontface = "bold"))
...
grid.text("Impact of HTE on %LnCVR (p<=0.05 &> 3-sigma) in small Samples: A Simulation Study",
          y = unit(1, "npc") - unit(0.00, "inches"),
          gp = gpar(fontsize = 12, fontface = "bold"))
