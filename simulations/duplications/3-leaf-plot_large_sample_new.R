...
simulate <- 1000
row_duplication_levels <- seq(0, 50, by = 10)
n_control <- 250
n_treatment <- 250
...
combined_plot <- p / p_prop / p_3sigma +
  ...
  plot_annotation(
    title = "Impact of Row Duplication on LnCVR: Large Sample (250/trial arm)",
...
# Save the combined plot with exact dimensions (8x11 inches)
# Get the current Windows username
username <- Sys.getenv("USERNAME")
combined_plot_path <- file.path("C:", "Users", username, "Dropbox", "Carlisle", "R-diabetes",
                                "Submission_manuscripts", "LnCVR-SAS", "LnCVR-publication_SAS", "Revision", "Duplications",
                                "combined_row_dup_plots_large.pdf")
...
cat("Flattened combined plot saved to:", paste0(combined_plot_path, "_flattened.pdf"), "\n")
