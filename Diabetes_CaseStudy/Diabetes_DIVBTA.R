...
master_postrandom |> 
  ...
  set_caption(as_paragraph(as_b("Characteristics of trials by presence or absence of post-randomization standard deviations"))) |> 
  save_as_docx(path="table1_postrandom_sd.docx")
...
master_problematicsd |> filter(problematicsd != "uhoh") |>
  ...
  bold_labels() |> 
  as_flex_table() %>% autofit() %>%
  set_caption(as_paragraph(as_b("Characteristics of trials by with and without problematic lnCVR"))) |> 
  save_as_docx(path="table1_problematicsd.docx")
